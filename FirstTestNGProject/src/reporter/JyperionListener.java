...
public class JyperionListener implements ITestListener {
	...
	public void onTestFailure(ITestResult result) {
		...
		String fileWithPath=ConstantVariables.fileWithPath;
	    WebDriver driver=LandingPage_Home.driver;
		try {
			TakeScreenshot.takeSnapShot(driver, fileWithPath);
		} catch (Exception e) {
		...
		if (throwable != null) {
			...
			this.nbExceptions++;
			 ...
			 Chunk imdb = new Chunk("[SCREEN SHOT]", new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE));
		        imdb.setAction(new PdfAction(fileWithPath));
		        Paragraph  excep = new Paragraph(
		            throwable.toString());
		        excep.add(imdb);
